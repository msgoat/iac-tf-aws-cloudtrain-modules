...
locals {
  ...
  alb_security_group_ids = length(var.loadbalancer_security_group_ids) != 0 ? concat([ aws_security_group.loadbalancer.id ], var.loadbalancer_security_group_ids) : [ aws_security_group.loadbalancer.id ]
}
