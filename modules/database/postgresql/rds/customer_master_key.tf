locals {
  kms_key_name        = "cmk-${local.db_instance_name}"
  kms_key_policy_name = "policy-cmk-${local.db_instance_name}-access"
}

# create a customer managed key to encrypt postgres data at rest and to encrypt postgres secrets
resource "aws_kms_key" "cmk" {
  description              = "customer master key of AWS RDS PostgreSQL cluster ${local.db_instance_name}"
  ...
  policy = jsonencode(
    {
      ...
      Statement = [
        ...
        {
          Sid      = "AllowCMKAccessFromRDSPostgreSQL"
          Effect   = "Allow"
          Principal = "*"
